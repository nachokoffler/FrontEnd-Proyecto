...
export class AltaReclusosComponent implements OnInit {
  ...
  cod_rec: number = 0;
  sentenciasSeleccionadas: any[] = []; // 👈 array para trackear seleccionadas
  ...
  agregarSentencia(sent: any) {
    const index = this.sentenciasSeleccionadas.indexOf(sent);
    if (index === -1) {
      this.sentenciasSeleccionadas.push(sent); // 
      this.respuesta.push(sent);
    } else {
      this.sentenciasSeleccionadas.splice(index, 1); // 
